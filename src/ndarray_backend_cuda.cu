...
__global__ void CompactKernel(const scalar_t* a, scalar_t* out, size_t size, CudaVec shape,
  ...
  /// BEGIN YOUR SOLUTION
    ...
    for (size_t i = 0; i < ndim; i ++) {
        prod_shape *= shape.data[i];
    }
    size_t index = offset;
    size_t gid_mod = gid;
    for (size_t i = 0; i < ndim; i ++) {
        prod_shape /= shape.data[i];
        size_t p = gid_mod / prod_shape;
        gid_mod = gid_mod % prod_shape;
        index += p * strides.data[i];
    }
    if (gid < size) out[gid] = a[index];
  /// END YOUR SOLUTION
}
...
__global__ void ReduceMaxKernel(const scalar_t* a, scalar_t* out, size_t reduce_size, size_t size) {
    size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
    if (gid < size) {
        scalar_t max_v = a[gid * reduce_size];
        for (int j = gid * reduce_size + 1; j < (gid + 1) * reduce_size; j ++) {
            max_v = max(max_v, a[j]);
        }
        out[gid] = max_v;
    }
...
__global__ void ReduceSumKernel(const scalar_t* a, scalar_t* out, size_t reduce_size, size_t size) {
    size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
    if (gid < size) {
        scalar_t sum_v = a[gid * reduce_size];
        for (int j = gid * reduce_size + 1; j < (gid + 1) * reduce_size; j ++) {
            sum_v += a[j];
        }
        out[gid] = sum_v;
